"""2D light painting - put lights on the plate without re-rendering it.

Ported from ComfyUI-ReLight (MIT) by the ComfyUI-ReLight authors.

This is the complement to the PBR relight nodes in this same package. Those
decompose an image into albedo/normal/depth and shade it from geometry; this
one paints light onto the plate directly, the way a comper does with a Radial
and a Grade - radial or directional falloff, colour or correction, rim light,
and mask-aware occlusion so a light behind the subject reads as behind it.

No model, no diffusion pass, deterministic.
"""
...
# SciPy does the mask blur, the rim-light distance transform and the occlusion
# smoothing. It is guarded because NukeMax's aggregator catches an import
# failure PER PACKAGE: an unguarded import here would take the whole relight
# package down - the PBR nodes with it - leaving one warning in a log nobody
# reads and six nodes missing from the menu with no explanation.
try:
...
class ReLight(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        ...
        """
        return io.Schema(
            node_id="NukeMax_ReLight2D",
            display_name="ReLight 2D (Light Paint)",
    ...
    def execute(cls, image: torch.Tensor, **kwargs) -> io.NodeOutput:
        ...
        # Checked here rather than at import: this node's package also holds
        # five math-only PBR nodes that do not need SciPy, and they must still
        # register when it is absent.
        if ndimage is None:
            raise RuntimeError(
                "ReLight 2D needs SciPy, which is not installed in this "
                f"ComfyUI environment ({_SCIPY_ERROR}). It does the mask blur, "
                "the rim-light distance transform and the occlusion smoothing - "
                "there is no usable fallback for those. Install it with: "
...
# NukeMax's aggregator reads these two dicts from every node package, so the
# upstream ComfyExtension entrypoint is replaced rather than kept alongside -
# two registration paths for one class is how a node ends up in the menu twice.
NODE_CLASS_MAPPINGS = {
    "NukeMax_ReLight2D": ReLight,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "NukeMax_ReLight2D": "ReLight 2D (Light Paint)",
}

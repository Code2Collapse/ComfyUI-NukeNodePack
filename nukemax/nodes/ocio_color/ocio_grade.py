# PORTED FROM: third_party/ComfyUI-OCIO/grade_nodes.py
# Licence: MIT — Copyright (c) 2026 Slava Sexton.
"""Arithmetic lift/gamma/gain/offset grade nodes (HDR-safe, no OCIO config required)."""
...
@resilient
class NukeMax_OCIOGrade:
    DESCRIPTION = (
        "Lift/gamma/gain/offset grade with contrast-about-pivot and luminance-preserving saturation; "
        "emits grade_info JSON for stamping the same correction elsewhere."
    )
    ...
    OUTPUT_TOOLTIPS = ("Image with the lift/gamma/gain/offset grade applied.",
                       "JSON grade_info describing this grade - wire into OCIO Apply Grade to stamp "
    ...
    def execute(self, image, lift_r, lift_g, lift_b, gamma_r, gamma_g, gamma_b, gain_r, gain_g, gain_b,
...
@resilient
class NukeMax_OCIOGradeMatch:
    DESCRIPTION = (
        "Shot-to-shot color match via Reinhard-style CIE Lab mean/std transfer, expressed as "
        "grade_info gain/offset for stamping across a sequence."
    )
    CATEGORY = "NukeMax/Color"
    FUNCTION = "execute"
    ...
    OUTPUT_TOOLTIPS = ("Image color-matched toward the reference.",
                       "JSON grade_info for the computed correction - wire into OCIO Apply Grade to "
                       "stamp the same match elsewhere.",
    ...
    def execute(self, image, reference, match_strength=1.0, mix=1.0, clamp_output=False):
        require_image_bhwc(image)
        require_image_bhwc(reference)
        ref = reference
...
@resilient
class NukeMax_OCIOApplyGrade:
    DESCRIPTION = (
        "Apply a grade_info JSON string from OCIO Grade or Grade Match to a new image, with optional "
        "strength lerp toward identity."
    )
    CATEGORY = "NukeMax/Color"
    FUNCTION = "execute"
    RETURN_TYPES = ("IMAGE",)
    ...
    def execute(self, image, grade_info, strength=1.0, mix=1.0, clamp_output=False):
        require_image_bhwc(image)
        grade, err = _grade_info_from_json(grade_info)
        if err:
            log.warning("OCIO Apply Grade: %s", err)
            return (image,)
...
NODE_CLASS_MAPPINGS = {
    "NukeMax_OCIOGrade": NukeMax_OCIOGrade,
    "NukeMax_OCIOGradeMatch": NukeMax_OCIOGradeMatch,
    "NukeMax_OCIOApplyGrade": NukeMax_OCIOApplyGrade,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "NukeMax_OCIOGrade": "OCIO Grade (Nuke)",
    "NukeMax_OCIOGradeMatch": "OCIO Grade Match (Nuke)",
    "NukeMax_OCIOApplyGrade": "OCIO Apply Grade (Nuke)",
}

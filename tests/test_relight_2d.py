"""ReLight 2D (light painting) - the upstream regression suite, ported.

Each test locks in a finding from the upstream audit. They are kept because
they encode real failures that were found and fixed once already, and a port
is exactly where those come back: the locked-clone fixture below exists
because ComfyUI calls a LOCKED SHALLOW CLONE of the node class, not the class
itself, and any per-run state written to the class raises AttributeError on
the first mask of every run. Testing the bare class cannot see that.

Ported from ComfyUI-ReLight (MIT).
"""
...
# ── fixtures ────────────────────────────────────────────────────────────────
#
# These live in this module rather than the shared conftest on purpose: they
# are named `node`, `run` and `defaults`, and `run` is already a fixture in
# test_par_aspect.py and test_reformat_aspect.py. Putting them in conftest.py
# would shadow those two and change what they measure, silently.
...
ReLight = pytest.importorskip(
    "nukemax.nodes.relight.light_paint",
    reason="ReLight 2D needs the ComfyUI v3 API, SciPy and PIL",
).ReLight
...
def test_shipped_example_workflows_match_the_schema(node):
    ...
    for path in workflows:
        ...
        for graph_node in graph["nodes"]:
            if graph_node["type"] != "NukeMax_ReLight2D":
                continue
...
def test_scaled_gamma_stays_within_the_widget_range(node):
    ...
    bounds = {
        (getattr(s, "min", None), getattr(s, "max", None))
...
def test_pixel_grid_is_cheap_to_cache(node):
    ...
    from nukemax.nodes.relight import light_paint as relight
...
def test_every_widget_default_is_within_its_declared_range(node):
    """A default outside its own min/max is a value the user cannot re-enter
    after changing it - the node works until you touch the widget, then never
    goes back.

    getattr throughout: socket inputs (image, mask) carry no `default` at all
    on the real API, so an attribute access would fail on the first one rather
    than checking the widgets that matter.
    """
    offenders = []
    for spec in node.define_schema().inputs:
        low, high = getattr(spec, "min", None), getattr(spec, "max", None)
        default = getattr(spec, "default", None)
        if default is None or low is None or high is None:
            continue
        if not (low <= default <= high):
            offenders.append((spec.id, default, low, high))
    assert offenders == []


def test_combo_defaults_are_offered_options(node):
    offenders = [
        (spec.id, getattr(spec, "default", None))
        for spec in node.define_schema().inputs
        if getattr(spec, "options", None) is not None
        and getattr(spec, "default", None) not in spec.options
    ]
    assert offenders == []
...
def test_preset_values_stay_within_widget_bounds(node):
    ...
    for preset_name, preset in node.PRESETS.items():
        for key, value in preset.items():
            low, high = getattr(specs[key], "min", None), getattr(specs[key], "max", None)
